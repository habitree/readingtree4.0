...
2. **`src/repositories/**`** (선택적 레이어, 있다면)
3. **`src/services/**`** (선택적 레이어, 있다면)
...
> **참고**: 현재는 `doc/database/`에 직접 위치하지만, 향후 `migrations/` 폴더로 정리 예정

### 4.3 파일명 규칙

```
YYYYMMDDHHmm__<기능명>__<변경내용>.sql
```

**예시:**
- `202412151430__notes__add_page_number_column.sql`
- `202412151500__books__add_isbn_index.sql`
- `202412151600__users__update_rls_policy.sql`
...
- `doc/database/migrations/YYYYMMDDHHmm__<기능명>__<변경내용>.sql`
...
3. **생성된 마이그레이션:**
   - `doc/database/migrations/202412151430__notes__add_page_number_column.sql`
```
...
└─ database/
   ...
   └─ migrations/ (SQL 마이그레이션 파일, 향후 정리 예정)
...
#### Step 1: DATA_MODEL.md 수정

```markdown
## Notes 테이블
- 목적: 사용자가 책에 대한 기록을 저장
- 변경: tags 컬럼 추가 (TEXT[], 배열 형태)
- 관계: user_id -> auth.users.id, book_id -> books.id
```

#### Step 2: 마이그레이션 파일 생성

`doc/database/migrations/202412151430__notes__add_tags_column.sql`:
...
export function useBooks() {
  ...
  const fetchBooks = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error("책 목록 조회 실패"));
    } finally {
      setIsLoading(false);
    }
  }, []);
